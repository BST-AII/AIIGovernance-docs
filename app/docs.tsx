...
const pages: Record<string, DocPage> = {
  ...
  installation: {
    ...
    sections: [
      ...
      { id: "linux", title: "Ubuntu 22.04 / 24.04 / WSL2", body: <><p>Ubuntu 桌面只需要 Git；安装包已携带 Python、Qt 和所需 XCB 运行库。WSL 必须启用 WSLg 才能显示窗口。请使用标注支持 Ubuntu 22.04+ 的最新安装包。</p><Code>{"sudo apt update\nsudo apt install -y git\ntar -xzf AIIGovernance-Setup-linux-x64-vX.Y.Z.tar.gz\ncd AIIGovernance-Setup-linux-x64-vX.Y.Z\nchmod +x AIIGovernance-Setup-linux-x64-vX.Y.Z\n./AIIGovernance-Setup-linux-x64-vX.Y.Z"}</Code><Note title="窗口打不开">先确认正在 Ubuntu 桌面或已启用 WSLg，并检查 <code>echo $DISPLAY</code>。不要在纯 SSH/headless 会话中期待 GUI。</Note><Note title="v0.3.31 在部分 Ubuntu 桌面提示 Could not initialize GLX">这是 Qt 无法使用当前显卡/远程桌面的 GLX 配置，不是 CPU 架构不匹配。仅使用 v0.3.31 且遇到该错误的用户，需要用下面的临时命令启动一次安装器；这些变量只影响本次启动，不会永久修改系统，也不是安装后的运行依赖。后续修复版会由安装器自动选择软件渲染，用户不再需要手动输入。</Note><Code>{"QT_OPENGL=software \\\nQT_QUICK_BACKEND=software \\\nQT_XCB_GL_INTEGRATION=none \\\nLIBGL_ALWAYS_SOFTWARE=1 \\\nQTWEBENGINE_CHROMIUM_FLAGS=--disable-gpu \\\n./AIIGovernance-Setup-linux-x64-v0.3.31"}</Code><p><code>libgvfscommon.so</code> 或 <code>libgvfsdbus.so</code> 的符号提示通常只是伴随警告；本问题的致命判据是随后出现 <code>Could not initialize GLX</code> 和 <code>Aborted</code>。</p></> },
