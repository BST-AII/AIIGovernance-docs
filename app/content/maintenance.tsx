  ...
  sections: [
    {
      ...
      body: <div className="simple-table">
        ...
        <div><b>Windows：安装到「Sync Agent 身份核验或安装」报 OSError: [Errno 9] Bad file descriptor</b><span>traceback 停在 import 阶段（<code>importlib … get_data</code>）——这不是代码错，是<b>文件被安全软件拿走了</b>。安装器把运行文件解压在 <code>%LOCALAPPDATA%\aiig-lite-installer-payload</code>，个别企业杀毒/终端防护/防火墙类软件会把其中个别 .py 判为可疑并<b>精准隔离</b>：真机案例里同目录十几个文件完好，唯独凭据加密模块 <code>platform_support.py</code> 消失（这类调用系统凭据接口的代码最容易被启发式误报）。自查一行：<code>dir &quot;%LOCALAPPDATA%\aiig-lite-installer-payload\payload-v2-*\python\hooks\aiig_agent&quot;</code>，列表里缺 <code>platform_support.py</code> 即中招。修复：让本机安全软件（或找 IT）把 <code>%LOCALAPPDATA%\aiig-lite-installer-payload</code> 加入排除/放行，然后<b>删掉整个该目录再重装</b>（残缺缓存不会自愈）。提示：若 <code>Add-MpPreference</code> 报 <code>0x800106ba</code>，说明 Defender 已被第三方安全软件接管、管事的不是它——排除项要加在真正在跑的那家里。解决前<b>不要反复重装</b>，每装一次就被隔离一次。属个别机器的安全策略问题，与安装包本身无关。</span></div>
        <div><b>运行官网 install.ps1 满屏红字：不支持「var」关键字、参数列表中缺少参量</b><span>这是把一张网页当成 PowerShell 在执行——报错里能看到 <code>script</code>、<code>crossorigin</code>、<code>function(o,c)</code> 这类网页代码，说明 <code>irm</code> 从 claude.ai 拿回来的不是安装脚本，而是被重定向后的 HTML（国内网络直连的典型结果）。<b>不要逐条去修语法错误</b>，脚本根本没到你手里。改用《快速开始》「安装前准备」里的国内装法：Node.js 18+ 加 <code>npm install -g @anthropic-ai/claude-code</code>，装完 <code>claude --version</code> 验证，再用管理员提供的 <code>setup.ps1</code> 导入 API Key。</span></div>
        <div><b>Linux：注册 Sync Agent 服务失败，报 <code>OPENSSL_3.4.0</code> not found（required by libsystemd-shared）</b><span>出现在 Ubuntu 25.04 / 24.10 这类<b>非 LTS 新系统</b>上（22.04 / 24.04 不受影响）。机制：安装器自带的运行库目录会随环境变量传给子进程，系统的 <code>systemctl</code> 被迫加载了安装器带的旧版 OpenSSL，而新系统的 systemd 需要更新的符号，于是 systemctl 自己起不来——错误里家目录下那个路径就是安装器的库目录。<b>换新版安装包解决不了这一个</b>，维护者已登记修复（records 仓 issue #28）。绕过：给安装器一个先清掉环境再转发的 systemctl。终端里执行：<Code>{`mkdir -p ~/aiig-fix
printf '#!/bin/sh\\nunset LD_LIBRARY_PATH\\nexec /usr/bin/systemctl "$@"\\n' > ~/aiig-fix/systemctl
chmod +x ~/aiig-fix/systemctl`}</Code>然后<b>从同一个终端</b>、带着改过的 PATH 启动安装器：<Code>{`PATH=~/aiig-fix:$PATH <安装器启动命令>`}</Code>装完把 <code>~/aiig-fix</code> 删掉即可。提醒两句：这类系统不在支持矩阵（我们承诺 22.04 / 24.04 LTS 与 WSL2），本条只是绕过；另外别用 v0.3.53 这类旧安装包，从下载页取最新版——旧包在 Linux 上还有一批后来才修掉的问题。</span></div>
    ...
    },
    {
      ...
      body: <>
        ...
        <div className="simple-table">
          ...
          <div><b>治理调用卡几分钟：record_triage / deliver_task 一直停在 calling</b><span>records 流水随每次工具调用增长、从不轮转，旧版每次治理调用都把整个 <code>events.jsonl</code> 从头读十几遍，文件到上百 MB 就是分钟级。先在会话里说「更新治理框架」拉到最新，然后<b>重开会话</b>——治理 MCP 是常驻进程，同步不会换掉正在跑的那个。仍慢的话看两样：治理 MCP 服务的错误输出里 <code>governance_mcp: slow tools/call …</code> 那一行给出工具名、耗时与两个流水文件的大小；PowerShell 跑 <code>dir records</code> 看 <code>events.jsonl</code> 多大。文件已上百 MB 时把大小和那行日志发给管理员：流水轮转在后续版本处理，当前版本只把读取路径改快了。</span></div>
        </div>
      </>,
    },
    {
    ...
    },
    {
      ...
      body: <div className="simple-table">
        <div><b>Mac：点「首次机器人验证」立刻报「未抓到发信人 open_id」</b><span><b>跟飞书无关，也不是你没发消息或者发慢了。</b>那一步会起一个临时的后台进程来接你的第一条消息，而它<b>起来就退了</b>，根本没人接，所以你发几次都一样。<b>确诊只要一条命令</b>：终端跑 <code>cat $TMPDIR/aiig-probe-*/probe.log</code>，看到 <code>failed to create agent … &quot;claude&quot; CLI not found in PATH</code> 就是这个原因。<b>为什么终端里 claude 明明能用、还报找不到</b>：安装器是双击启动的图形程序，macOS 不会把你 <code>.zshrc</code> 里的 PATH 交给它，所以它只认五个固定目录——<code>~/.local/bin</code>、<code>~/.claude/local</code>、<code>~/.npm-global/bin</code>、<code>/usr/local/bin</code>、<code>/opt/homebrew/bin</code>；<b>用 npm、nvm、conda 装的 claude 都不在这五个里面</b>。<b>修法</b>：终端跑 <code>command -v claude</code> 找出它在哪（没有输出就用 <code>find ~ -maxdepth 6 \( -name claude -type f -o -name claude -type l \) 2&gt;/dev/null</code> 搜一遍），<b>先确认 <code>&lt;路径&gt; --version</code> 真能跑</b>，再把它链进候选表的第一个目录：<code>mkdir -p ~/.local/bin &amp;&amp; ln -s &lt;路径&gt; ~/.local/bin/claude</code>，然后重新点「首次机器人验证」。<b>两个坑要留意</b>：① <b>VS Code 扩展自带的那份不算装了命令行版本</b>（路径形如 <code>~/.vscode/extensions/anthropic.claude-code-*/resources/native-binary/claude</code>）——在 IDE 里天天用 Claude Code，不等于这台机器上有 <code>claude</code> 命令；而且扩展一升级就换目录，链过去迟早会断，<b>断掉时的现象跟现在一模一样</b>。没有命令行版本就装一个：<code>curl -fsSL https://claude.ai/install.sh | bash</code>，它正好装进 <code>~/.local/bin</code>。② <b>nvm 的路径里带 node 版本号</b>（形如 <code>v24.14.0</code>），以后升级 node 时软链同样会断，现象也一样。<b>还有个办法</b>：从终端启动安装器而不是双击，它就继承了你 shell 的 PATH；但下次双击又不行，所以还是做软链省事。<b>顺手清一下</b>：每重试一次都会在临时目录留下一份配置，里面有飞书 <code>app_secret</code> 明文，那是用完即弃的，确诊完可以删：<code>rm -rf $TMPDIR/aiig-probe-*</code>。</span></div>
        ...
        <div><b>安装最后一步报「20 秒内没有看到当前项目状态」</b><span><b>多半跟你正在装的这个项目无关。</b>后台同步代理是整台机器共用一个，它启动时会先把<b>所有</b>已装项目过一遍；只要其中<b>任何一个</b>项目的目录读不出来（公司加密沙箱、网络盘、被安全软件接管的目录），代理就会整个退出，于是新项目的状态文件永远写不出来，安装器等满 20 秒报这一句。真机现场（2026-09-23）：一台机器因此<b>所有</b>项目的回流停了三周，直到装第三个项目才暴露。<b>修法：升级到 0.3.72 及以上</b>，读不了的项目会被自动跳过，其余项目照常。升级后若<b>这个项目自己</b>仍装不上，说明它本身在沙箱里——需要让 IT 把 Records Agent 的 python 可执行文件加入白名单；在那之前它不会回流，但不再影响别的项目。</span></div>
        <div><b>某个项目的 <code>records-sync-status.json</code> 里出现 <code>blocked</code></b><span>这是 0.3.72 起的如实标注：这个项目的本地文件读不出来，回流已对<b>它</b>停止，<b>其他项目不受影响</b>。里面的 <code>detail</code> 会写明是哪个路径、什么错误。<b>0.3.73 起你不用自己翻这个文件</b>——如果被跳过的正是你刚装的那个项目，安装器装完会当场把这三句话说给你听（哪个项目不回流、什么原因、下一步做什么）；在那之前那种情况会显示成一次普通的「安装完成」，看不出问题。最常见的是公司加密沙箱——源码目录只允许白名单里的程序读取，而后台代理不在名单上。处置是找 IT 加白名单；判断方法：用 PowerShell 列一下那个目录（<code>Get-ChildItem &lt;路径&gt;</code>），如果 PowerShell 列得出来、而代理读不到，那就是按进程管的沙箱。</span></div>
        <div><b>装完后同步一直静默：平台上看不到这台机器的任何回流</b><span><b>0.3.72 起先看一眼项目里的 <code>.governance/records-sync-status.json</code> 有没有 <code>blocked</code></b>（有就看上一条）。没有的话再按下面查：PowerShell 跑 <code>C:\Windows\System32\schtasks.exe /query /tn &quot;AIIGovernance Records Sync Agent&quot; /v /fo list</code>，看「要运行的任务」一行。<b>路径里含 <code>.tmp-</code> 字样即确诊</b>：某次安装曾在中途被安全策略打断（典型是 IT 未及时加白名单时报 WinError 5），自启动任务指向了残缺的临时目录，同步代理每次开机启动即退。修法一步：<b>重跑安装器选「重装」</b>——任务路径会被强制改写为完整环境；装完再查一遍该命令确认不含 <code>.tmp-</code>。顺手删掉缓存目录 <code>%LOCALAPPDATA%\aiig-lite-installer-payload</code> 下名字带 <code>.tmp-</code> 的文件夹（非必须，重装后它不再有害）。注意：路径正常但也没回流，多半只是还没归档过任务——回流只在治理任务归档时发生，不是故障。</span></div>
        <div><b>机器人或知识回流每隔三天准时消失：登录后头三天正常，之后无声停掉，直到下次重新登录</b><span>Windows 对没有写运行时限的计划任务默认「运行超过 72 小时就停止」，而机器人（<code>cc-connect</code>）和同步代理（<code>AIIGovernance Records Sync Agent</code>）这两个任务在 0.3.68 及更早版本注册时都没有指定时限，所以登录第三天整点会被系统停掉，停掉后没有任何东西再把它拉起来——台式机不注销就一直停着；笔记本还多一条：任务默认「用电池时不启动、切到电池就停止」，拔掉电源也会下线。判据：PowerShell 跑 <code>C:\Windows\System32\schtasks.exe /query /tn cc-connect /v /fo list</code>（同步代理把任务名换成 <code>&quot;AIIGovernance Records Sync Agent&quot;</code>），「Stop Task If Runs X Hours and X Mins」是 <code>72:00:00</code>、「Last Result」是 <code>267014</code> 即坐实。处置按顺序：①<b>升级到 0.3.69 及以后</b>，安装器会自动去掉时限和电池限制，机器人不用重新配置、升级时在原页面点「复用现有安装」重启一次即可；②暂时不能升级就手动改——<b>以管理员身份</b>打开 PowerShell 粘贴下面这段（普通 PowerShell 会报 Access is denied）：<code>{'foreach ($n in \'cc-connect\',\'AIIGovernance Records Sync Agent\') { $t = Get-ScheduledTask -TaskName $n -ErrorAction SilentlyContinue; if (-not $t) { continue }; $t.Settings.ExecutionTimeLimit = \'PT0S\'; $t.Settings.DisallowStartIfOnBatteries = $false; $t.Settings.StopIfGoingOnBatteries = $false; Set-ScheduledTask -InputObject $t | Out-Null; schtasks /End /TN $n; schtasks /Run /TN $n }'}</code>；不想敲命令就打开「任务计划程序」找到这两个任务 → 属性 → 「设置」页取消勾选「如果任务运行时间超过以下时间，停止任务」、「条件」页取消两项电池选项，然后右键结束再运行。手动改过的设置在下次重装或升级机器人时会被打回默认，届时再改一次或直接升 0.3.69。③只想马上救活、暂不改设置：<code>schtasks /Run /TN cc-connect</code>（同步代理同理）拉起，三天后会再停。</span></div>
        <div><b>机器人突然没反应了 / 桌面上多出一个黑色终端窗口</b><span><b>那个窗口就是机器人服务本体，关掉窗口 = 停掉机器人。</b>Win11 默认终端（Windows 终端）不理会启动参数里的隐藏窗口选项，服务只能以可见窗口运行——看着像忘了关的黑窗口，顺手一关服务就没了，且不会自愈。判据：PowerShell 跑 <code>C:\Windows\System32\schtasks.exe /query /tn cc-connect /v /fo list</code>，「上次结果」是 <code>-1073741510</code>（即 0xC000013A，进程被 Ctrl+C/关窗口终止）就是这个原因。恢复：<code>C:\Windows\System32\schtasks.exe /run /tn cc-connect</code>，然后在飞书里发条消息验证。治本二选一：① 以后看到 cc-connect 窗口点最小化、不要关；② Windows 设置 → 系统 → 开发者选项 → 「终端」（默认终端应用程序）改为 <b>Windows 控制台主机</b>——改完后服务按登录自启动时窗口真正隐藏，没有窗口可误关（代价是自己开 PowerShell 也会用回旧式控制台外观）。已在用户机上验证有效。</span></div>
        <div><b>凭据校验报 <code>URLError: urlopen error timed out</code></b><span>网络层超时，凭据还没被检查——安装器/服务是直连飞书接口的，浏览器能打开 <code>open.feishu.cn</code> 只说明浏览器走了公司代理，Python 进程读不懂 PAC 自动配置脚本、也处理不了要域账号认证的代理，直连就被防火墙拦下。自证：PowerShell 跑 <code>curl.exe -m 5 https://open.feishu.cn</code> 同样超时即坐实。处置：向 IT 索取代理地址，把 <code>HTTPS_PROXY</code>/<code>HTTP_PROXY</code> 写进<b>系统级</b>环境变量（系统属性 → 环境变量 → 系统变量）再重新启动安装器或服务——只在当前终端里临时 <code>$env:HTTPS_PROXY=...</code> 的话，登录自启动的服务拿不到它，重启后会再犯。此步为可选集成，不急可先跳过、装完后再接。</span></div>
      </div>,
    },
    {
