  ...
  sections: [
    {
      ...
      body: <div className="simple-table">
        ...
        <div><b>Windows：安装到「Sync Agent 身份核验或安装」报 OSError: [Errno 9] Bad file descriptor</b><span>traceback 停在 import 阶段（<code>importlib … get_data</code>）——这不是代码错，是<b>文件被安全软件拿走了</b>。安装器把运行文件解压在 <code>%LOCALAPPDATA%iig-lite-installer-payload</code>，个别企业杀毒/终端防护/防火墙类软件会把其中个别 .py 判为可疑并<b>精准隔离</b>：真机案例里同目录十几个文件完好，唯独凭据加密模块 <code>platform_support.py</code> 消失（这类调用系统凭据接口的代码最容易被启发式误报）。自查一行：<code>dir &quot;%LOCALAPPDATA%iig-lite-installer-payload\payload-v2-*\python\hooksiig_agent&quot;</code>，列表里缺 <code>platform_support.py</code> 即中招。修复：让本机安全软件（或找 IT）把 <code>%LOCALAPPDATA%iig-lite-installer-payload</code> 加入排除/放行，然后<b>删掉整个该目录再重装</b>（残缺缓存不会自愈）。提示：若 <code>Add-MpPreference</code> 报 <code>0x800106ba</code>，说明 Defender 已被第三方安全软件接管、管事的不是它——排除项要加在真正在跑的那家里。解决前<b>不要反复重装</b>，每装一次就被隔离一次。属个别机器的安全策略问题，与安装包本身无关。</span></div>
